...
import { formatRoomStatus } from "@/admin/utils/formatLabels";
...
export default function RegisterRoomView() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validationError) {
      showError(validationError, { numero: form.numero });
      setSaving(false);
      return;
    }
    ...
    try {
      ...
      showError(err.message, { numero: form.numero });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <section className="space-y-6">
      <h1 className="text-2xl font-semibold text-emerald-900">Registrar habitación</h1>
        ...
        <div className="rounded-xl border border-emerald-100 bg-white p-6 shadow-sm">
          ...
          {loading ? (
            ...
            <ul className="max-h-[420px] space-y-2 overflow-y-auto">
              {rooms.map((room) => (
                <li key={room.id} className="rounded-lg border border-gray-200 px-3 py-2 text-sm">
                  <span className="font-medium">Hab. {room.room_number}</span>
                  <span className="ml-2 text-gray-500">{formatRoomStatus(room.status)}</span>
                </li>
              ))}
            </ul>
          )}
